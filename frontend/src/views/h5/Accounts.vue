        ...
        >
          <div class="accounts-list">
            ...
            <template v-if="item.type !== 'archived'">
              <div v-for="account in getAccountsForDisplay(item.group.accounts)" :key="account.fullPath || account.name" class="account-item">
                <van-cell
                  ...
                  @click="account.hasChildren ? toggleAccountExpand(account) : undefined"
                >
                  <template #icon>
                    <div class="account-icon" :style="{ marginLeft: (account.level || 0) * 16 + 'px' }">
                      <van-icon 
                        v-if="account.hasChildren" 
                        :name="account.isExpanded ? 'arrow-down' : 'arrow'" 
                        class="expand-icon"
                      />
                      <van-icon :name="getAccountIcon(account.subtype || account.type)" />
                    </div>
                  </template>
                  <template #right-icon>
                    <van-button
            ...
            <!-- 已归档账户 -->
            <template v-else>
              <van-empty v-if="filteredArchivedAccounts.length === 0" description="暂无已归档账户" />
              <div
        ...
        <van-form @submit="handleCreateAccount">
          ...
          <van-field
            ...
            :rules="[
              ...
              { pattern: /^[A-Z0-9][\w\u4e00-\u9fa5-:]*$/, message: '账户名称必须以大写字母或数字开头' }
            ]"
          />
          ...
          <div class="form-actions">
            <van-button round block type="primary" native-type="submit" :loading="createLoading">
              创建账户
            </van-button>
          </div>
        </van-form>
...
const orderedAccountGroups = computed(() => {
  const groups = searchText.value ? filteredAccountGroups.value : accountGroups.value
...
const filteredAccountGroups = computed(() => {
  ...
  Object.keys(accountGroups.value).forEach(type => {
    const filteredAccounts = accountGroups.value[type].accounts.filter(account =>
      account.name.toLowerCase().includes(searchText.value.toLowerCase()) ||
      (account.fullPath && account.fullPath.toLowerCase().includes(searchText.value.toLowerCase()))
    )
...
const getAccountsForDisplay = (accounts: Account[]) => {
  ...
  // 递归构建显示列表，跳过第一级
  const buildDisplayList = (accountsList: Account[], skipFirstLevel = true): Account[] => {
    ...
    accountsList.forEach(account => {
      ...
      if (skipFirstLevel && account.level === 0 && account.fullPath?.split(':').length === 1) {
        // 直接显示子账户
...
const handleCreateAccount = async () => {
  try {
    ...
    const requestData: AccountCreateRequest = {
      ...
      currencies: createForm.value.currencies ? [createForm.value.currencies] : undefined,
...
const loadAccounts = async () => {
  try {
    ...
    allAccountGroups.value = typeof accountData === 'object' && accountData !== null ? accountData : {}
    ...
    if (availableTypes.length > 0) {
      ...
      if (availableTypes.includes('Assets')) {
        ...
        // 选择第一个非archived的类型
        const nonArchivedType = availableTypes.find(type => type !== 'archived')
