              ...
              <template v-for="node in accountTree" :key="node.path">
                ...
                >
                  <van-icon 
                    ...
                    class="expand-icon"
                  />
                  <van-icon :name="getAccountIcon(node.path)" class="node-icon" />
            ...
            <template v-for="node in categoryTree" :key="node.path">
              ...
              >
                <van-icon 
                  ...
                  class="expand-icon"
                />
                <van-icon :name="getCategoryIcon(node.path)" class="node-icon category-icon" />
                <span class="node-name">{{ node.displayName }}</span>
              </div>
...
import { getAllAccounts, getAccountsByType } from '@/api/accounts'
...
const filteredAccounts = computed(() => {
  ...
  if (props.showAccountTypes) {
    filtered = filtered.filter(account => 
      account && account.name && account.name.startsWith(`${activeAccountType.value}:`)
    )
  }
...
const getCategoryIcon = (categoryName: string) => {
  ...
  if (name.includes('餐饮') || name.includes('食品') || name.includes('meals') || name.includes('food')) return 'food-bank-card'
  if (name.includes('交通') || name.includes('车费') || name.includes('transport') || name.includes('travel')) return 'logistics'
  if (name.includes('购物') || name.includes('商品') || name.includes('shopping') || name.includes('goods')) return 'shopping-cart-o'
  if (name.includes('娱乐') || name.includes('电影') || name.includes('entertainment') || name.includes('movie')) return 'music'
  if (name.includes('住房') || name.includes('房租') || name.includes('housing') || name.includes('rent')) return 'home-o'
  if (name.includes('医疗') || name.includes('健康') || name.includes('medical') || name.includes('health')) return 'medic'
  if (name.includes('教育') || name.includes('学习') || name.includes('education') || name.includes('study')) return 'education'
  if (name.includes('投资') || name.includes('理财') || name.includes('investment') || name.includes('finance')) return 'chart-trending-o'
  if (name.includes('服装') || name.includes('clothing') || name.includes('apparel')) return 'gift-o'
  if (name.includes('通讯') || name.includes('手机') || name.includes('communication') || name.includes('phone')) return 'phone-o'
  if (name.includes('水电') || name.includes('公用') || name.includes('utilities')) return 'fire-o'
  return 'apps-o'
}
...
const buildTreeFromCategories = (categoryList: Category[]) => {
  ...
  categoryList.forEach(category => {
    ...
    for (let i = 1; i < parts.length; i++) { // 从索引1开始，跳过顶级
      ...
      if (!nodeMap.has(path)) {
        const hasChildrenInList = categoryList.some(cat => 
...
const navigateToLevel = (level: number) => {
  if (level < currentPath.value.length - 1) {
    currentPath.value = currentPath.value.slice(0, level + 1)
    // 重新加载对应级别的分类
    loadCategoriesForPath()
  }
}
...
const loadCategoriesForPath = () => {
  ...
  if (currentPath.value.length === 0) {
    ...
    for (const pathItem of currentPath.value) {
      const found = categories.find(c => c.name === pathItem.fullPath)
      if (found && found.children) {
        categories = found.children
      } else {
        // 如果找不到对应的子分类，可能是路径错误，回到根级别
...
const loadAccounts = async () => {
  try {
    ...
    if (accountData && typeof accountData === 'object') {
      ...
      console.log('FullScreenSelector - Liabilities账户:', accountData.Liabilities)
      ...
      if (props.accountTypes.includes('Liabilities') && accountData.Liabilities) {
      ...
      console.warn('FullScreenSelector - 账户数据格式不正确或为空:', accountData)
    }
    
    // 转换为Account格式
    accounts.value = accountsList
      .filter((accountName: string) => accountName && typeof accountName === 'string')
    ...
    console.error('FullScreenSelector - 错误详情:', (error as any).response || (error as any).message || error)
    showToast('加载账户列表失败')
